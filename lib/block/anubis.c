...
)
{
	...
	/* last round */
	t0 = (s0[x0 >> 24] & 0xFF000000U) ^
		(s1[x1 >> 24] & 0x00FF0000U) ^
		(s2[x2 >> 24] & 0x0000FF00U) ^
		(s3[x3 >> 24] & 0x000000FFU) ^
		k[i++];

	t1 = (s0[(x0 >> 16) & 0xFF] & 0xFF000000U) ^
		(s1[(x1 >> 16) & 0xFF] & 0x00FF0000U) ^
		(s2[(x2 >> 16) & 0xFF] & 0x0000FF00U) ^
		(s3[(x3 >> 16) & 0xFF] & 0x000000FFU) ^
		k[i++];

	t2 = (s0[(x0 >> 8) & 0xFF] & 0xFF000000U) ^
		(s1[(x1 >> 8) & 0xFF] & 0x00FF0000U) ^
		(s2[(x2 >> 8) & 0xFF] & 0x0000FF00U) ^
		(s3[(x3 >> 8) & 0xFF] & 0x000000FFU) ^
		k[i++];

	t3 = (s0[x0 & 0xFF] & 0xFF000000U) ^
		(s1[x1 & 0xFF] & 0x00FF0000U) ^
		(s2[x2 & 0xFF] & 0x0000FF00U) ^
		(s3[x3 & 0xFF] & 0x000000FFU) ^
		k[i++];
...
)
{
	...
	if(key_len > ANUBIS_MAX_KEY) return -1;
	...
	for(r = 0; r <= s->r; r++)
	{
		...
		{
			t0 = ks0[kx[i] >> 24] ^
				(ks1[t0 >> 24] & 0xFF000000U) ^
				(ks1[(t0 >> 16) & 0xFF] & 0x00FF0000U) ^
				(ks1[(t0 >> 8) & 0xFF] & 0x0000FF00U) ^
				(ks1[t0 & 0xFF] & 0x000000FFU);

			t1 = ks0[(kx[i] >> 16) & 0xFF] ^
				(ks1[t1 >> 24] & 0xFF000000U) ^
				(ks1[(t1 >> 16) & 0xFF] & 0x00FF0000U) ^
				(ks1[(t1 >> 8) & 0xFF] & 0x0000FF00U) ^
				(ks1[t1 & 0xFF] & 0x000000FFU);

			t2 = ks0[(kx[i] >> 8) & 0xFF] ^
				(ks1[t2 >> 24] & 0xFF000000U) ^
				(ks1[(t2 >> 16) & 0xFF] & 0x00FF0000U) ^
				(ks1[(t2 >> 8) & 0xFF] & 0x0000FF00U) ^
				(ks1[t2 & 0xFF] & 0x000000FFU);

			t3 = ks0[kx[i] & 0xFF] ^
				(ks1[t3 >> 24] & 0xFF000000U) ^
				(ks1[(t3 >> 16) & 0xFF] & 0x00FF0000U) ^
				(ks1[(t3 >> 8) & 0xFF] & 0x0000FF00U) ^
				(ks1[t3 & 0xFF] & 0x000000FFU);
		}
