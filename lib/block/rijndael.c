...
/*
 * rijndael128-alg-fst.c
 *
 * @version 3.0 (December 2000)
 *
 * Optimised ANSI C code for the rijndael128 cipher (now AES)
 *
...
#define AES_MAX_ROUNDS 52
#define AES_DEFAULT_ROUNDS 6
#define AES_MAX_KEY 32

#define AES_ROUNDS(r, klen) ((r) + ((klen + 3) >> 2))
#define AES_K_LEN(R) (((R) + 1) << 2)
...
)
{
	...
	for(i = 0; (k - s->k + n) < AES_K_LEN(s->r); k += n)
	{
		t = k[n - 1];
		k[n] = k[0] ^
			(te4[(t >> 16) & 0xFF] & 0xFF000000) ^
			(te4[(t >> 8) & 0xFF] & 0x00FF0000) ^
		...
		for(j = 1; j < n && (k - s->k + n + j) < AES_K_LEN(s->r); j++)
		{
			if(n >= 8 && j == 5)
			{
				...
				k[n + 4] = k[n >> 1] ^
					(te4[t >> 24] & 0xFF000000) ^
					(te4[(t >> 16) & 0xFF] & 0x00FF0000) ^
					(te4[(t >> 8) & 0xFF] & 0x0000FF00) ^
					(te4[t & 0xFF] & 0x000000FF);
			}
			k[n + j] = k[j] ^ k[n + j - 1];
		}
	}
	...
	for(i = 4; i < (s->r << 2); i++)
	{
		s->dk[i] = td0[te4[s->dk[i] >> 24] & 0xFF] ^
			...
			td2[te4[(s->dk[i] >> 8) & 0xFF] & 0xFF] ^
			td3[te4[s->dk[i] & 0xFF] & 0xFF];
	}
...
)
{
	...
	/* last round */
	t0 = (te4[x0 >> 24] & 0xFF000000) ^
		(te4[(x1 >> 16) & 0xFF] & 0x00FF0000) ^
		(te4[(x2 >> 8) & 0xFF] & 0x0000FF00) ^
		(te4[x3 & 0xFF] & 0x000000FF) ^ s->k[i++];

	t1 = (te4[x1 >> 24] & 0xFF000000) ^
		(te4[(x2 >> 16) & 0xFF] & 0x00FF0000) ^
		(te4[(x3 >> 8) & 0xFF] & 0x0000FF00) ^
		(te4[x0 & 0xFF] & 0x000000FF) ^ s->k[i++];

	t2 = (te4[x2 >> 24] & 0xFF000000) ^
		(te4[(x3 >> 16) & 0xFF] & 0x00FF0000) ^
		(te4[(x0 >> 8) & 0xFF] & 0x0000FF00) ^
		(te4[x1 & 0xFF] & 0x000000FF) ^ s->k[i++];

	t3 = (te4[x3 >> 24] & 0xFF000000) ^
		(te4[(x0 >> 16) & 0xFF] & 0x00FF0000) ^
		(te4[(x1 >> 8) & 0xFF] & 0x0000FF00) ^
		(te4[x2 & 0xFF] & 0x000000FF) ^ s->k[i++];
...
)
{
	...
	/* last round */
	t0 = (td4[x0 >> 24] & 0xFF000000) ^
		(td4[(x3 >> 16) & 0xFF] & 0x00FF0000) ^
		(td4[(x2 >> 8) & 0xFF] & 0x0000FF00) ^
		(td4[x1 & 0xFF] & 0x000000FF) ^ s->dk[i++];

	t1 = (td4[x1 >> 24] & 0xFF000000) ^
		(td4[(x0 >> 16) & 0xFF] & 0x00FF0000) ^
		(td4[(x3 >> 8) & 0xFF] & 0x0000FF00) ^
		(td4[x2 & 0xFF] & 0x000000FF) ^ s->dk[i++];

	t2 = (td4[x2 >> 24] & 0xFF000000) ^
		(td4[(x1 >> 16) & 0xFF] & 0x00FF0000) ^
		(td4[(x0 >> 8) & 0xFF] & 0x0000FF00) ^
		(td4[x3 & 0xFF] & 0x000000FF) ^ s->dk[i++];

	t3 = (td4[x3 >> 24] & 0xFF000000) ^
		(td4[(x2 >> 16) & 0xFF] & 0x00FF0000) ^
		(td4[(x1 >> 8) & 0xFF] & 0x0000FF00) ^
		(td4[x0 & 0xFF] & 0x000000FF) ^ s->dk[i++];
...
)
{
	...
	{
		case 4:
			if(r > AES_MAX_ROUNDS) return 0;
			break;

		case 5:
			if((r / 6) << 2 > AES_MAX_ROUNDS) return 0;
			break;
		...
		case 8:
			if(r >> 1 > AES_MAX_ROUNDS) return 0;
			break;
